...
class BurnManexStoreViewModel @Inject constructor(
    ...
    val showMoreManexStoreData: LiveData<Resource<ShopResultDto>> = Transformations.switchMap(_requestDto) {
        shopRepository.shopList(it)
    }

    val sortData: LiveData<Resource<List<SortDto>>> =
        featureListRepository.loadSortListData(SortType.RedeemShopSort)
    ...
    val loadFilterData: LiveData<Resource<FilterDto>> = Transformations.map(
        featureListRepository.loadFilterListData(PageFilterType.RedeemShopFilter)) { res ->
    ...
    val getFilterCount : LiveData<Resource<CountResultDto>> = Transformations
        .switchMap(_filterRequestDto) { input ->
            shopRepository.shopCount(input)
        }
