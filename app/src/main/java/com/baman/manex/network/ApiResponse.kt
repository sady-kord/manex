...
sealed class ApiResponse<T> {
    ...
    companion object {
        ...
        fun <T> create(response: Response<ResultDto<T>>): ApiResponse<T> {

            return if (response.isSuccessful && null != response.body() && response.body()?.code == 200) {

                val body = response.body()

                if (body == null || response.code() == 204 || body.data == null) {
