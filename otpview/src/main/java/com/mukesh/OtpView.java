...
public class OtpView extends AppCompatEditText {
  ...
  public OtpView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    errorBackground = typedArray.getDrawable(R.styleable.OtpView_errorBackground);
  ...
  protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
    ...
    if(text.length() == 0 && onOtpCompletionListener !=null){
      onOtpCompletionListener.onOtpClear();
    }
    makeBlink();
